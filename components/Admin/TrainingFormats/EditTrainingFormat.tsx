...
const EditTrainingFormat: React.FC<PropType> = ({ refetch, name, id }) => {
  ...
  const submitData = (values: FormType) => {
    ...
    api
      .put(`/formats/${id}`, values)
      .then((res) => {
        refetch();
        handleClose();
        notify("Training Format updated successfully !", "success");
        reset();
      })
      .catch((err) => {
        setLoading(false);
...
export default EditTrainingFormat;
